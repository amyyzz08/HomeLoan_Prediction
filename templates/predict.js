d3.select("#predict").on("click", () =>{
    ...
    d3.json(`/predict/${gender}/${married}/${dependents}/${self_employed}/${loan_amount}/${credit_history}/${property_area}/${total_income}`, (predicted) => { 
        var loan_status = Math.floor(predicted.prediction)
        var loan_status_text = ""
        if(loan_status = 1){
            loan_status_text = `Congratulations! You are elibigle for this loan.`
        } else if (loan_status = 0){
            loan_status_text = `Your application for this loan will decline`
        } else { 
            loan_status_text = `Not enough information to determine your loan status`
        }

        d3.select("#forcasted-status").html(`
